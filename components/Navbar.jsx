  ...
  return (
    <nav className={styles.navbar}>
      ...
      {isMobile ? (
        <>
          {isScrolled &&  (
            <div className={styles.btnContainer} onClick={handleBookNow}><button>Book Now</button></div>
          )}
          ...
          {isOpen && (
            <div className={styles.dropDown} onClick={() => setIsOpen(false)}>
              {navItems.map((item, i) => (
                <p onClick={()=>handleScroll(item.link,i)} key={i} className={styles.dropDownItem}>
                  {item.name}
                </p>
              ))}
            </div>
